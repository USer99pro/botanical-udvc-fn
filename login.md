...
### Login Modal
- **Purpose:** Provide one entry point for administrators and staff to sign in.
- **Container:** Use a centered modal with a maximum width of `28rem`, a `rounded-3xl` surface, a soft shadow, and a dark translucent backdrop with blur.
- **Header:** Center-align the title, organization description, and role badges for `ผู้ดูแลระบบ (Admin)` and `เจ้าหน้าที่ (Staff)`.
- **Username Input:** Accept an email address or username and include a leading person icon.
- **Password Input:** Use a password field with a leading lock icon.
- **Admin Icon:** Place the `admin_panel_settings` icon in a centered `56px` rounded container directly below the password input and above the submit button. It must remain visually centered at all viewport sizes.
- **Submit Button:** Use a full-width primary pill button with a login icon and the label `เข้าสู่ระบบ (Login)`. Show a loading indicator and reduce opacity while the request is in progress.
- **Validation and Errors:** Keep validation feedback inside the modal above the form. Use the error color tokens without hiding failed login messages.
- **Registration:** Keep a secondary text link below the form: `ยังไม่มีบัญชีผู้ใช้งาน? สมัครสมาชิกใหม่ที่นี่`. Closing the modal before navigating to registration prevents stacked overlays.
- **Accessibility:** Provide a clear close button, visible labels, keyboard-focusable controls, and an `aria-label` for icon-only controls.
